...
// You have to define here your real destination server without a trailing slash (eg: http://dashboard.tuxweb.it)
//$real_destination_host = "http://dtp-24.sncs.abdn.ac.uk:8080/ecosystem-transport/";
//$real_destination_host = "http://localhost:8080/ecosystem-transport/";
//$real_destination_host = "http://cops-022382.uoa.abdn.ac.uk:8080/ecosystem-transport/";
//$real_destination_host = "http://cops-022256.uoa.abdn.ac.uk:8080/ecosystem-quality/";
$real_destination_host = "http://dtp-126.sncs.abdn.ac.uk:8080/ecosystem-quality/";
...
function proxypass($url, $method, $parameters, $header_timeout_ms, $body_timeout_ms) {
	...
	if (($method == "POST") || ($method == "PUT") || ($method == "DELETE")) {
		...
		$opts['http']['content'] = str_replace( "\\\"", "\"", $parameters['stuff']);  
		//$opts['http']['Content-length'] = strlen($parameters['stuff']);
//		$opts['http']['content'] = http_build_query($parameters['data']);
	}
	...
	if (!$handle) {
		...
		echo ": caused by: ";
		echo " hmm" . $opts['http']['content'] . "asdf";
...
// CLIENT SIDE PROXY
$url = $real_destination_host . $parameters['service'] . "/" . $parameters['method'] . $_SERVER['REQUEST_URI'];
